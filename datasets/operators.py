...
def result_compute(num_all_list, exp_tokens):
    ...
    for id, step in enumerate(step_list):
        ...
        for item in ANNO.mid_varible_dict:
            if item[0] in ['N', 'V'] and isinstance(parse_latex(ANNO.mid_varible_dict[item]).evalf(), Float):
                mid_value =  float(parse_latex(ANNO.mid_varible_dict[item]).evalf())
                if mid_value<=0:
                    print("N or V is not pos!")
                    raise Exception 
